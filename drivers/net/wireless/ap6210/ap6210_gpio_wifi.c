...
static char *wifi_para = "sdio_wifi_bpi_para"; /*modify by BPI team for ap6210*/
...
static unsigned int ap6210_wl_vdd_en = 0; /*add by BPI team for ap6210*/
...
static int ap6210_gpio_ctrl(char* name, int level)
{
	...
	int ret = 0;
	int gpio = 0;
	char * gpio_name[3] = {"ap6210_wl_regon", "ap6210_bt_regon", "ap6210_wl_vdd_en"}; /*modify by BPI team for ap6210*/
	...
	for (i = 0; i < 3; i++) {
		if (strcmp(name, gpio_name[i]) == 0) {
			...
			case 1: /*ap6210_bt_regon*/
				...
				break;
			case 2: /*add by BPI team for ap6210*/
	...
	if (ret) /*add by BPI team for ap6210*/
	{
...
*/
   ...
   /* The comment must remove when we use the Bluetooth<BPI>
...
static int ap6210_gpio_wifi_get_res(void)
{
	struct ap6210_gpio_wifi_ops *ops = &ap6210_wifi_select_pm_ops;
	 ...
	 /*add by BPI team for AP6210*/
	if (SCRIPT_PARSER_OK != script_parser_fetch(wifi_para, "sdio_wifi_bpi_used", &ops->wifi_used, 1)) {
		AP6210_ERR("parse sdio_wifi_bpi_used failed in script.fex.\n" );
		return -1;
	}
	if (!ops->wifi_used) {
	...
	/*add by BPI team for AP6210*/
	if (SCRIPT_PARSER_OK != script_parser_fetch(wifi_para, "sdio_wifi_bpi_sdc_id", &ops->sdio_id, 1)) {
		AP6210_ERR("parse sdio_wifi_bpi_sdc_id in script.fex failed.\n" );
		return -1;
	}
/* remove by BPI team for AP6210
...
*/
	/*add by BPI team for AP6210*/
	if (SCRIPT_PARSER_OK != script_parser_fetch(wifi_para, "sdio_wifi_bpi_mod_sel", &ops->module_sel, 1)) {
		AP6210_ERR("parse sdio_wifi_bpi_mod_sel in script.fex failed.\n" );
		return -1;
	}
